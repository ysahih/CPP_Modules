...
ClapTrap &ClapTrap::operator=(ClapTrap &a) {
	...
	std::cout << "Parent copy operator assignment called!" << std::endl;
    this->_name = a._name;
...
ClapTrap::ClapTrap(std::string name) : _name(name){
	...
	std::cout << "Parent Parametrized Constructor called" << std::endl;
	this->_hit_points = 10;
	this->_energy_points = 10;
	this->_attack_damage = 0;
}
...
void ClapTrap::attack(const std::string &target ) {
	...
	if (this->_hit_points <= 0) {
		std::cout << this->_name << " has no enough Hit Points!" << std::endl;
		return ;
	}
